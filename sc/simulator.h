...
#define llf_debug if(pConfig->bDebug) cerr
...
private:
    ...
    //llf add begin: Move original code of coalesce from xOverNode to a function.
    void llf_xOverCoalesce(NodePtr& xOverNode, bool bAllowOut=true);

    double getRecomPos(uint k);

    //store compination point of each local tree
    vector<double> reComPosition;
    ...
    EdgePtr selectRandomEdgeAtHeight(EdgePtrVector& edgeVector, double height);//llf added
